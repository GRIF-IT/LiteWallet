...
const char DEFAULT_WALLET_FILE_NAME[] = "investcoin.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultNodeList << "node1.investcoin.pro:5334" << "node2.investcoin.pro:5334" << "explorer.investcoin.pro:5334" ;
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("investcoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("investcoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("investcoinwallet.desktop");
  ...
  res = autorunSettings.contains("investcoinwallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("investcoinwallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "pro.investcoinvanets.investcoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Investcoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("investcoinwallet", appPath);
    } else {
      autorunSettings.remove("investcoinwallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\Investcoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:investcoinwanec");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\Investcoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\Investcoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("investcoinwallet.cfg"));
